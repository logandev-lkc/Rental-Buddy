...
> **v2**：路徑 `v2/inbody-report-gen-spec.md`。全目錄索引見 [README.md](../README.md)。
...
**實作對齊（Rental Buddy Web，2026-05-09）**：查核項以 `ItemState.selectedOptions`（細節 chips，規格稱 **answers**）與 `quickStatus`（良好／可接受／需注意／尚未確認）承載結構化輸入；`itemOptionConfig` 覆蓋率為合約 5/7、設備 7/10、安全 5/7、生活 4/5、鄰里 5/6，各分類皆達 ≥70%。**Phase 2 收尾（同日）**：AI 匯出 JSON `checklistTable` 每列含 **`answerType`**（`multiSelect`＝有細節 options；`singleSelect`＝以快速狀態為主）、**`answers`**（與 `selectedOptions` 相同）、**`itemId`**；備註仍為展開後補充說明。
...
**實作對齊（Rental Buddy Web，2026-05-09）**：痛點面向已多以「合併題」涵蓋——室內氣味 **n5**、蟲害 **n6**、浴室通風／乾濕 **s7**、收納 **f10**、插座電力 **f9**、採光／對外窗 **s1**、洗曬 **f5**，避免題數膨脹；後續若需拆項再另開版本評估。
...
**實作對齊（Rental Buddy Web，2026-05-09）**：「同區候選平均」優先依 **比較清單**（`compareIds`）內其他紀錄計算；若清單為空則退回「其他有進度紀錄」概估。報告 KPI 文案為 **同區候選平均**，並附 **candidateAverageHint**（強調非市場平均）；JSON／AI Prompt 亦帶入 `candidateAverageBasis`。
